    ...
    QTimer::singleShot(2000, this, [this]() {
        if (_pCommandSocket->state() != QAbstractSocket::ConnectedState) {
            ...
            emit errorState(
                ...
                static_cast<int>(QAbstractSocket::SocketTimeoutError)
                );
        }
    });
    connect(_pCommandSocket, &QTcpSocket::connected, this, [=](){
        qDebug() << "连接成功";
        connect(_pCommandSocket,SIGNAL(readyRead()),this,SLOT(on_read_command()));
    });
    connect(_pCommandSocket, &QAbstractSocket::errorOccurred, this, [=](QAbstractSocket::SocketError err){
        qDebug() << "连接失败";
